...
class CassandraMirrorContext
  ...
  def executeAction[O](cql: String, bind: Row => Row = identity, generated: Option[String] = None) =
    ActionMirror(cql, bind(Row()))

  case class BatchActionMirror(cql: String, bindList: List[Row])

  def executeActionBatch[T, O](cql: String, bindParams: T => Row => Row = (_: T) => identity[Row] _, generated: Option[String] = None) = {
    val f = (values: List[T]) =>
